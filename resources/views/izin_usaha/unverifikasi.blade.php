...
@section('content')
                ...
                <div class="card shadow-sm border-0">
                    ...
                    <div class="card-body">
                        <div class="alert alert-info mb-4">
                            Halaman ini menampilkan pengajuan izin usaha berstatus <strong>SUBMIT / Belum
                                Diverifikasi</strong>.
                            Silakan lakukan verifikasi dengan memeriksa data usaha, dokumen surat, dan foto usaha.
                            Pilih <strong>Hasil Verifikasi</strong> (Lolos atau Tolak), tentukan <strong>OPD
                                Tujuan</strong>,
                            serta isi <strong>Catatan Verifikasi</strong>, kemudian klik tombol
                            <span class="badge bg-success">Verifikasi</span> untuk menyimpan hasil verifikasi.
                        </div>
                        <div class="table-responsive">
                            <table class="table table-hover mb-0" id="tabelIzinUsahaUnverifikasi" style="width: 100%;">
    ...
    <!-- Modal Verifikasi Izin Usaha -->
    <div class="modal fade" id="modalVerifyIzinUsaha" tabindex="-1" aria-labelledby="modalVerifyIzinUsahaTitle"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content border-0 shadow">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title" id="modalVerifyIzinUsahaTitle"></h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"
                        aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="formVerifyIzinUsaha" method="POST">
                        @csrf
                        <input type="hidden" name="izin_usaha_id" id="izin_usaha_id">

                        <div class="mb-3">
                            <label class="form-label fw-semibold">Hasil Verifikasi <span
                                    class="text-danger">*</span></label>
                            <div>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="is_verified"
                                        id="is_verified_yes" value="1">
                                    <label class="form-check-label" for="is_verified_yes">Lolos</label>
                                </div>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="is_verified"
                                        id="is_verified_no" value="0">
                                    <label class="form-check-label" for="is_verified_no">Tolak</label>
                                </div>
                            </div>
                            <div class="text-danger small mt-1" id="error-is_verified"></div>
                        </div>

                        <div class="mb-3">
                            <label for="catatan_verifikasi" class="form-label fw-semibold">Catatan Verifikasi <span
                                    class="text-danger">*</span></label>
                            <textarea class="form-control" name="catatan_verifikasi" id="catatan_verifikasi" rows="4"
                                placeholder="Masukkan catatan verifikasi minimal 10 karakter"></textarea>
                            <div class="text-danger small mt-1" id="error-catatan_verifikasi"></div>
                        </div>

                        <div class="mb-3" id="opdWrapper">
                            <label for="opd_id" class="form-label fw-semibold">OPD Tujuan</label>
                            <select class="form-select" name="opd_id" id="opd_id">
                                <option value="">Pilih OPD tujuan</option>
                                @foreach ($opds ?? [] as $opd)
                                    <option value="{{ $opd->id }}">{{ $opd->nm_opd }}</option>
                                @endforeach
                            </select>
                            <div class="text-danger small mt-1" id="error-opd_id"></div>
                        </div>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            $('#tabelIzinUsahaUnverifikasi').DataTable({
                processing: true,
                serverSide: true,
                destroy: true,
                order: [
                ...
                displayLength: 25,
                ajax: {
                    url: BASE_URL + "/verifikasi/get-datatables-unverifikasi",
                },
                columns: [{
                        orderable: false,
                        data: function(data) {
                    ...
                    //                 return `
                // <div class="text-left">
                //     <strong>No Pengajuan: ${data.nomor_pengajuan}</strong><br>
                //     <strong>Nama Usaha: ${data.nama_usaha}</strong><br>
                //     <small class="text-muted">Tanggal dibuat: ${date.toLocaleDateString('id-ID')}</small><br>
                //     Status:
                //     <span class="badge ${badgeClass}">${data.status}</span><br>
                //     Tahapan saat ini:
                //     <span class="badge bg-secondary">${data.tahapan_saat_ini}</span><br>
                //     ${alertBadge}<br>OPD Tujuan: ${data.nm_opd ? data.nm_opd : '-'}
                // </div>`;
                    //             }
                    //         },
                    {
...

                    {
                        orderable: false,
                        data: function(data) {
                            const canVerify = data.status === 'SUBMIT';
                            return `
                    ...
                    <ul class="dropdown-menu dropdown-menu-end">
                        <li>
                            <button class="dropdown-item" type="button"
                                onclick="openVerifyModal(this)" data-id="${data.id}"
                                ${!canVerify ? 'disabled' : ''}>
                                <i class="bi bi-check2-circle text-success"></i> Verifikasi
                            </button>
                        </li>
                    </ul>
                </div>
        ...
        $('input[name="is_verified"]').on('change', function() {
            if ($(this).val() === '1') {
                $('#opdWrapper').show();
            } else {
                $('#opdWrapper').hide();
        ...
        function openVerifyModal(el) {
            $('#formVerifyIzinUsaha')[0].reset();
            $('#izin_usaha_id').val($(el).data('id'));
            $('.text-danger').html('');
            $('.form-control, .form-select, .form-check-input').removeClass('is-invalid');
            $('#opdWrapper').hide();
            $('#modalVerifyIzinUsahaTitle').text('Verifikasi Pengajuan Izin Usaha');
            $('#modalVerifyIzinUsaha').modal('show');
        }

        $('#formVerifyIzinUsaha').on('submit', function(e) {
            e.preventDefault();

            $('.text-danger').html('');
            $('.form-control, .form-select, .form-check-input').removeClass('is-invalid');

            const id = $('#izin_usaha_id').val();
            const formData = $(this).serialize();

            $.ajax({
                url: BASE_URL + '/verifikasi/verify/' + id,
                type: 'POST',
                data: formData,
                dataType: 'JSON',
                success: function(res) {
                    if (res.success) {
                        ...
                        $('#modalVerifyIzinUsaha').modal('hide');
                        $('#tabelIzinUsahaUnverifikasi').DataTable().ajax.reload(null, false);
                    } else {
